#ファイル名：file_check_by_timestamp_add_grep.ps1
#機能説明：任意のフォルダ配下のファイルを任意の日時（日・時・分）前以降のファイルのみ抽出する＋GREP
#
#起動コマンド例（C:\workフォルダ（サブフォルダも含む）以下の拡張子classの最終更新日が現在日時より0日2時間45分前以降のファイルより文言「”abc”」がヒットする行をGREPして前1行後ろ2行を表示する）
#.\file_check_by_timestamp_add_grep.ps1 C:\work class $TRUE 0 2 45 “`”abc`”” 1 2
Param([string] $paramDirectory, [string] $paramFileType, [boolean]$isFindSubDirectory = $FALSE, [int]$paramFromDays = 0, [int]$paramFromHours = 0, [int]$paramMinutes = 0, [string] $paramPettern, [int]$paramBefore = 0, [int]$paramAfter = 0)
...
foreach($x in $a) {
	...
	if(!$isSkip){
		...
		Select-String $paramPettern (dir $x) -Context $paramBefore,$paramAfter
	}
}
